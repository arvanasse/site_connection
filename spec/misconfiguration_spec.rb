...
describe Koinonia::SiteConnection do
  before :all do
    @no_site_defined = {
      :environment => 'test',
      :load_path   => File.join( File.dirname(__FILE__), '..', 'config', 'no_sample.yml' )
    }

    @no_environment_defined = {
      :environment => 'test',
      :load_path   => File.join( File.dirname(__FILE__), '..', 'config', 'no_test.yml' )
    }
  end

  describe 'calling establish_site_connection with improper configuration' do
    ...
    it 'should raise an argument error if the requested connection is not defined for the Rails environment' do
      lambda{ RemoteResource.establish_site_connection(:sample, @no_site_defined) }.
        should raise_error(ArgumentError, "Sample is not defined for test in #{@no_site_defined[:load_path]}")
    end
